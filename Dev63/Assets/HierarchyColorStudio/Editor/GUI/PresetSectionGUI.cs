...
namespace CryNet.HierarchyColorStudio
{
    ...
    internal sealed class PresetSectionGUI
    {
        ...
        /// <param name="maxHeight">Maximum height of the scrollable preset list.</param>
        internal void Draw(HierarchyColorStore store, IReadOnlyList<GameObject> selection, float maxHeight)
        {
            if (store == null)
                return;
        ...
        private void DrawHexField(HierarchyColorStore store, ColorPreset preset)
        {
            ...
            if (editing && !ColorHex.TryParse(m_HexEditValue, out _))
            {
                var rect = GUILayoutUtility.GetLastRect();
                StudioStyles.DrawOutline(rect, new Color(0.85f, 0.35f, 0.3f, 0.9f), 1f);
            }
        }
